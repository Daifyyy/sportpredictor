...
4. **Statistiky** — Celkový přehled (3 metriky nahoře), pak 2 sub-taby `⚽ Výsledky & Góly` | `🔄 Rohy`. Každý sub-tab: (1) tabulka Počet/Správně/Špatně/Úspěšnost % s radio přepínačem `Podle typu | Podle ligy`; podmíněné formátování ≥60% zelená/45–60% žlutá/<45% červená. (2) reliability line charty (beze změny). Žádné bar charty.
